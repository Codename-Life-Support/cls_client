...
static char* g_dxgi_present_address = nullptr;
static char* g_content_check_address = nullptr;
...
bool HooksInit() {
    ...
    g_dxgi_present_address = (char*)dxgi_module + DXGI_Present_offset;
    if (MH_CreateHook(g_dxgi_present_address, &DX_Render, (void**)(&send_func_ptr)) != MH_OK) {
        MessageBoxA(0, "[CLS-CLIENT] could not set hook at IDXGISwapChain::Present -> dxgi.h", "DirectX hook failure", 0);
        return false;
    }
    if (MH_EnableHook(g_dxgi_present_address) != MH_OK) {
    ...
    g_content_check_address = (char*)exeModule + 0x25157B;
    if (MH_CreateHook(g_content_check_address, &AsmDetour0, (void**)(&player_content_check_ptr)) != MH_OK) {
        MessageBoxA(0, "[CLS-CLIENT] could not set hook at MCC checkplayercontent", "MCC hook failure", 0);
        return false;
    }
    if (MH_EnableHook(g_content_check_address) != MH_OK) {
